    ...
    for index_ms2, row in data_ms2.iterrows():
		...
		#data_xic, status = pyMZML_xic_out(loc, float(tol / (10 ** 6)), time_w - h_rt_w, time_w + h_rt_w,row['mz'],run_temp, runid_list,rt_list)
                ...
                if flag_windows:
                    ...
                    args_txic = shlex.split(os.path.join(moff_path, "txic") + " " + mz_opt + " -tol=" + str(tol) + " -t " + str(time_w - h_rt_w) + " -t " + str(time_w + h_rt_w) + " " + loc )
...
def main_apex_alone():
    ...
    parser.add_argument('--rt_p_match', dest='rt_p_window_match', action='store', type=float, default=1.5,
                        help='specify the time windows for the matched  peak ( minute). Default value is 1.5 ',
                        required=False)
